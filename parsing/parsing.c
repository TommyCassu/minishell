...
/*   Updated: 2025/06/17 23:54:53 by tcassu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	verif_other_tokens(t_shell *shell, t_token *tokens)
{
	...
	while (tmp)
	{
		if (tmp && (tmp->type == PIPE || tmp->type == L_REDIRECT
				|| tmp->type == R_REDIRECT
				|| tmp->type == HEREDOC || tmp->type == APP_REDIRECT))
		{
			...
			if ((ft_strncmp(tmp->value, "<", 2) == 0
					&& (tmp->next->type != L_REDIRECT
						&& tmp->next->type != WORD)))
				return (syntax_error(shell, "newline"));
			if (tmp->next->type != WORD && tmp->next->next->type != WORD)
